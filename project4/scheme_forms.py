...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        if not isinstance(expressions.first,Pair):
            ...
            return '{0}'.format(expressions.first)
        else:
            env.define(expressions.first.first,scheme_eval(expressions.rest,env))
            return '{0}'.format(expressions.first.first)
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        new_pair=Pair(signature.rest,expressions.rest)
        a=do_lambda_form(new_pair,env)
        env.bindings['{0}'.format(signature.first)]=a
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    a=LambdaProcedure(formals,expressions.rest,env)
    return a
    # END PROBLEM 7
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    part=bindings.first
    while part is not nil:
        validate_form(part,2,2)
        names=Pair(part.first,names)
        if self_evaluating(part.rest.first):
            values=Pair(part.rest.first,values)
        else:
            values=Pair(scheme_eval(part.rest.first,env),values)
        if bindings.rest is nil:
            break
...
def do_mu_form(expressions, env):
    ...
    validate_formals(formals)
    # BEGIN PROBLEM 11
    a=MuProcedure(formals,expressions.rest)
    return a
    # END PROBLEM 11
