...
def count_change(total, next_money):
    ...
    def f(a,b):
        if a==0:
            ...
            return 0
        elif next_money(b) and a>=next_money(b):
            return f(a-b,b)+f(a,next_money(b))
        else:
            return f(a-b,b)
    return f(total,1)
...
def multiadder(n):
    ...
    def adder_out(n, x):
        def adder(y):
            if n == 1:
                return x + y
            return adder_out(n - 1, x + y)
        return adder
    return adder_out(n, 0)
