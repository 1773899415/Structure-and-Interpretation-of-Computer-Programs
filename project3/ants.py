...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        a=self.place
        i=0
        while i<self.min_range and not a.is_hive:
            a=a.entrance
            i=i+1
        n=self.min_range
        while not a.is_hive and self.min_range<=n<=self.max_range:
            if a.bees:
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.chew_countdown==0 and self.place.bees:
            ...
            Insect.reduce_health(a,a.health)
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN PROBLEM 8
        if self.ant_contained!=None and not self.is_tank:
            self.ant_contained.action(gamestate)
        elif self.is_tank and self.ant_contained==None:
            b=list(self.place.bees)
            for a in b:
                Insect.reduce_health(a,self.damage)
        elif self.is_tank and self.ant_contained!=0:
            b=list(self.place.bees)
            for a in b:
                Insect.reduce_health(a,self.damage)
            self.ant_contained.action(gamestate)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        a=self.place.exit
        while a!=None:
            if a.ant!=None and a.ant.is_container:
                if a.ant.is_buff==False:
                    a.ant.damage=a.ant.damage*2
                    a.ant.is_buff=True
                if a.ant.ant_contained!=None:
                    if a.ant.ant_contained.is_buff==False:
                        a.ant.ant_contained.damage=a.ant.ant_contained.damage*2
                        a.ant.ant_contained.is_buff=True
            elif a.ant!=None:
                if a.ant.is_buff==False:
                    a.ant.damage=a.ant.damage*2
                    a.ant.is_buff=True
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN PROBLEM 12
        Ant.reduce_health(self,amount)
        if self.place.ant.health==0 or self.place.ant.is_container:
            ants_lose()
        # END PROBLEM 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        elif self.health > 0 and destination is not None and self.scary_step==0 and self.slow_step==0:
            self.move_to(destination)
        elif self.slow_step>0 and self.scary_step==0:
            self.slow_step=self.slow_step-1
            if gamestate.time%2==0:
                self.move_to(destination)
        elif self.slow_step==0 and self.scary_step>0:
            destination=self.place.entrance
            self.scary_step=self.scary_step-1
    ...
    def slow(self, length):
        ...
        # BEGIN PROBLEM EC
        self.slow_step=self.slow_step+length
        # END PROBLEM EC
    ...
    def scare(self, length):
        ...
        """
        # BEGIN PROBLEM EC
        self.scary_step=self.scary_step+length
        # END PROBLEM EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN PROBLEM EC
        if target and target.have_not_been_scary:
            target.have_not_been_scary=False
            target.scare(2)
        # END PROBLEM EC
